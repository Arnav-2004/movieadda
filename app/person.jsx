...
const person = () => {
  ...
  const getWorkedInDetails = async (id) => {
    const data = await fetchPersonWorkedIn(id);

    if (data && data.cast) setWorkedIn(data.cast);
  };
        ...
        <View>
          <View className="flex-row justify-center">
            <View className="items-center rounded-full overflow-hidden h-64 w-64 border-2 border-neutral-500">
              <Image
                source={{ uri: imgW342(person?.profile_path) }}
          ...
          <MovieList title="Known For" data={workedIn} hideSeeAll={true} />
        </View>
      )}
    </ScrollView>
  );
